...
void OpDispatchBuilder::VCVTPH2PSOp(OpcodeArgs) {
  ...
  const auto DstSize = GetDstSize(Op);
  const auto SrcLoadSize = Op->Src[1].IsGPR() ? DstSize : DstSize / 2;
...
void OpDispatchBuilder::VCVTPS2PHOp(OpcodeArgs) {
  ...
  LOGMAN_THROW_A_FMT(Op->Src[1].IsLiteral(), "Src[1] needs to be a rounding-mode literal");
  const auto Imm8 = Op->Src[1].Data.Literal.Value;
